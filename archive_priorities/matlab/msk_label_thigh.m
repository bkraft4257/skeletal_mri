function msk_label_thigh( inT2w )
%% Initialization

if nargin<1 || isempty(inT2w)
    inT2w='t2w';
end
...
n4FileName = sprintf('%s_n4.nii',inT2w);

if(~(exist(n4FileName,'file')==2))
    
    fprintf('\n >>>> Applying N4BiasFieldCorrection to %s.nii \n', inT2w );
...
% Find
atLccThreshold = graythresh(atLcc{1});  % Select threshold
atLcc{2}    = 0*atLcc{1};
...
save_untouch_nii(atLccNii, sprintf('%s_n4_atLCC.nii', inT2w))
...
clear sm;
sm{1}    = atLcc{end};
sm{2}    = imopen(sm{1}, strel('ball',5,3));
sm{3}    = sm{2}>0.5;
...
sm{9}   = sm{8} .* (1-atVessels{end});  % Remove vessels
sm{10}  = zeros(size(sm{9}));

% Find largest connected component on each slice
for ii=1:nSlices
    sm{10}(:,:,ii) = local_largest_connected_component(sm{9}(:,:,ii));
end

[sm{11}, sm{12} ] =  local_activeContour3D(sm{10}, inMuscleSmoothFactor, 50);
...
save_untouch_nii(labelsNii, sprintf('%s_n4_labelsAuto.nii', inT2w))
