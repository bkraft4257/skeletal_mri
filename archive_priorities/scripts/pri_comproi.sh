...
WriteHdr() {
	...
	if (( MeanOnly == 0 && Lesion == 0 )) ; then
	    echo -n ",${lab}-std,${lab}-nVoxels, ${lab}-Volume" >> "$ROImeas"
	fi
    done
    echo "" >> "$ROImeas"
}
...
for (( i=0 ; i<$nImg ; i++ )) ; do
	...
	if (( Lesion == 0 )) ; then
	    ...
	    if (( MeanOnly == 0 )) ; then
		nV1=`fslstats ${Tmp}wk -V | awk '{print $1}'`
		nV2=`fslstats ${Tmp}wk -V | awk '{print $2}'`
		Sd=`fslstats ${Tmp}wk -S | sed 's/^[ ]*//;s/[ ]*$//;' | awk '{print $1 / 100 }'`
		echo -n ",$Sd,$nV1, $nV2" >> "$ROImeas"
	    fi
	else
