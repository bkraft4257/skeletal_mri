...
def extract_volumes_from_raw_images( rawPcasl, nM0, verboseFlag=False ):
...
def mcflirt( rawPcasl, verboseFlag=False ):
   
    from nipype.interfaces.fsl import MCFLIRT
 
    mcflirt = MCFLIRT()
    
    mcflirt.inputs.in_file    = "cl."+rawPcasl
...
def plot_motion_parameters( rawPcasl, verboseFlag=False ):

    import nipype.interfaces.fsl as fsl

    plotter                  = fsl.PlotMotionParams()
    plotter.inputs.in_file   = "mcf.cl." + rawPcasl +".par"
...
if __name__ == "__main__":
     ...
     parser.add_argument("--m0_scale",      help="M0 scale (default = 0.00001)", type=float, default=0.00001 )
     parser.add_argument("--m0_index",      help="M0 scale (default = 89)", type=int, default=8x )
 


     parser.add_argument("--refscarehashs", help="Reference Scans (default = [87 94])", type=int, default= [ 87, 94 ] )
     ...
     input_files = [[ inArgs.raw,     ":colormap=grayscale:grayscale=0,4096" ],
...
#             mcflirt_filename = mcflirt( inArgs.raw, inArgs.verbose )
#
#             plot_motion_parameters( inArgs.raw, inArgs.verbose )

             extracted_filenames = extract_volumes_from_raw_images( inArgs.raw, inArgs.verbose )

             m0_filename      = extracted_filenames[0]
             clPairs_filename = extracted_filenames[1]

             mean_m0 = calc_mean_m0( m0_filename, inArgs.m0_mask, inArgs.verbose, inArgs.debug)

             print mean_m0

             pwi_filename = 'spwi_02.nii.gz'
             subprocess.call(["ImageMath","4", pwi_filename, "TimeSeriesSimpleSubtraction", clPairs_filename])

             iwUtilities.iw_subprocess([ "fslmaths", pwi_filename, "-div",  str(mean_m0), "-div", 
                                         str(inArgs.m0_scale), pwi_filename ], inArgs.verbose)
    

             out_pwi_filename  = "spwi_" + str(2*inArgs.nclavg[0]).rjust(2,'0') + ".nii.gz"
